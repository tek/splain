...
trait SplainFormattingExtension extends typechecker.splain.SplainFormatting with SplainFormattersExtension {
  ...
  override def formatImplicitChain(chain: List[ImplicitError]): List[String] = {
    lazy val _compact = formatImplicitChainTreeCompact(chain)
    lazy val _full = formatImplicitChainTreeFull(chain)

    if (_compact.getOrElse(Nil).size > _full.size) {
      println("error!")
    }

    val compact = if (settings.VimplicitsVerboseTree) None else _compact
    compact.getOrElse(_full)
  }
...
//      val spacer = if (leaves.nonEmpty && leaves.length + 1 < chain.length) List("⋮".blue) else Nil
