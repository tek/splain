...
trait Formatting extends Formatters with ImplicitMsgCompat { self: Analyzer =>
  import global._
  ...
  def extractArgs(tpe: Type) =
    tpe match {
      case PolyType(params, result) =>
        result.typeArgs.map {
          case t if params.contains(t.typeSymbol) =>
            WildcardType
          case a =>
            a
        }
      case t: AliasTypeRef if !isAux(tpe) =>
        t.betaReduce
          .typeArgs
          .map(a =>
            if (a.typeSymbolDirect.isTypeParameter)
              WildcardType
            else
              a,
          )
  ...
  def ctorNames(tpe: Type): List[String] =
    scala
      .util
      .Try(tpe.typeConstructor.toString)
      .map(_.split('.').toList)
  ...
  def formatAuxSimple(tpe: Type): (List[String], String) = {
    ...
    val num =
      if (names.lift(names.length - 2).contains("Case"))
        3
      else
        2
  ...
  case class MatchRewrite(target: String) {
    def unapply(candidate: (Regex, String)): Option[String] = {
      ...
      if (transformed == target || transformed.isEmpty)
        None
      else
        Some(transformed)
    }
  ...
  def flattenInfix(tpe: Infix): List[Formatted] = {
    def step(tpe: Formatted): List[Formatted] =
      tpe match {
  ...
  def truncateDecls(decls: List[Formatted]): Boolean = featureTruncRefined.exists(_ < decls.map(_.length).sum)
  ...
  def showFormattedQualified(path: List[String], name: String): TypeRepr = {
    ...
    val regexRewritten =
      rewriteRegexes
        .foldLeft(fullType) { case (current, (regex, replacement)) =>
          regex.replaceAllIn(current, replacement)
        }
    val stripped =
      if (fullType == regexRewritten || regexRewritten.isEmpty)
        stripModules(path, name)(featureKeepModules)
      else
        regexRewritten
    FlatType(stripped)
  }
  ...
  val specialFormatters: List[SpecialFormatter] = List(
    FunctionFormatter,
    TupleFormatter,
  ...
  def formatNestedImplicit(err: ImpFailReason): (String, List[String], Int) = {
    ...
    val reason =
      err match {
        case e: ImpError =>
          implicitMessage(e.param)
        case e: NonConfBounds =>
  ...
  def indentTree(tree: List[(String, List[String], Int)], baseIndent: Int): List[String] = {
    ...
    tree
      .flatMap { case (head, tail, nesting) =>
  ...
  def formatImplicitChainTreeCompact(chain: List[ImpFailReason]): Option[List[String]] =
    chain
      ...
      .map { head =>
        ...
        val spacer =
          if (leaves.nonEmpty && leaves.length < chain.length)
            List("⋮".blue)
          else
            Nil
  ...
  def formatImplicitChainFlat(chain: List[ImpFailReason]): List[String] =
    chain.map(formatNestedImplicit).flatMap { case (h, t, _) =>
      h :: t
    }
  ...
  def formatImplicitChainTree(chain: List[ImpFailReason]): List[String] = {
    val compact =
      if (featureCompact)
        formatImplicitChainTreeCompact(chain)
      else
        None
  ...
  def formatImplicitChain(chain: List[ImpFailReason]): List[String] =
    if (featureTree)
      formatImplicitChainTree(chain)
    else
      formatImplicitChainFlat(chain)
  ...
  def overrideMessage(msg: String): Option[String] =
    if (msg.startsWith(Messages.lazyderiv))
      None
    else
      Some(msg)
